...
KEY = 'divngefkdpqlcmferfxef3fr'  # 加密所用的秘钥
...
def package(route):
    ...
    for dirpath, dirnames, filenames in zip_files:
        for filename in filenames:
            if filename == "zipTool.py" or filename == zip_file_name:  # 防止打包脚本和资源文件在同一个目录
                continue
...
def setup_package(info={}):
    ...
    entrance = "index.html"
    global loaclversion
    loaclversion = input("请输入版本号(当前版本" + str(info[VERSION]) + "):") if VERSION in info else input("请输入版本号:")

    package_info = {
        ROUTE: route,
        ENTRANCE: entrance,
        VERSION: loaclversion
    }
...
print("打包完成，请更新以下sql:")
#print('\033[1;31;40m')
#print("UPDATE tbl_app_info SET `version`='{version}',down_url='https://b10image.cfsicao.com/client/client.zip?={version}' ,current_host='https://b10image.cfsicao.com/client/client.zip?={version}' WHERE id=1;".format(version=loaclversion))
print("UPDATE tbl_app_info SET `version`='{version}',down_url='https://b10image.cnmshzh.com/client/client.zip?={version}' ,current_host='https://b10image.cfsicao.com/client/client.zip?={version}' WHERE id=1;".format(version=loaclversion))
    #print('\033[1;31;40m')
    deploy(path)
