...
//    func requestVersionBlock( version: String,downloadUrl:NSURL) -> String?
    func requestVersionBlock( version: String,downloadUrl:NSURL) -> String?
...
//typealias requestVersionBlock = () -> Int
//typealias funcBlock = () -> () //或者 () -> Void
public class WebView: WKWebView {

    // MARK: - Public
    typealias requestVersionBlock  = (String) ->();
    var callBack : requestVersionBlock?;
...
//        } else {
        ...
        NSLog("readAccessURL.path---%@", readAccessURL.path);
...
//            NSLog("webapp--%@", webapp.version);
            let downloadUrl = Router.shared.downloadUrl(with: routeUrl)
             self.load(url: URL(fileURLWithPath: webapp.localPath))
            if let delegate = self.delegate, let versionMes = delegate.requestVersionBlock(version: webapp.version ,downloadUrl:downloadUrl! as NSURL)
            {
                
                NSLog("versionMes--%@", versionMes);
