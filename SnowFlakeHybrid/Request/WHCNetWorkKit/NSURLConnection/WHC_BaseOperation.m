...
- (void)setStrUrl:(NSString *)strUrl {
   ...
   // _Headers=nil;
    ...
    [self setHeaderdic:self.Headers];
}
-(void)setHeaderdic:(NSDictionary*)headers
{
    
    
    [_urlRequest setValue:[NSObject getUserToken] forHTTPHeaderField:@"accessToken"];
    [_urlRequest setValue:[NSObject deviceIPAdress] forHTTPHeaderField:@"ip"];
    NSString *userid = [NSObject getUserID] ;
    [_urlRequest setValue:userid forHTTPHeaderField:@"userId"];
    if (headers!=nil) {
        for (id key in headers.allKeys) {
            id value =[headers objectForKey:key];
            if ([value isKindOfClass:[NSNumber class]]) {
                [_urlRequest setValue:[value stringValue] forHTTPHeaderField:key];
