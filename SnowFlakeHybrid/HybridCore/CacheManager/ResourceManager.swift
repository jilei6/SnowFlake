...
internal class ResourceManager: NSObject {
    ...
    /// 查询一个资源包信息
    public func appVersion(withRoute url: String) -> String? {
       // let webapp = webapp(withRoute: routeUrl),
      let webapp = selectWebapp(routeUrl: url)
        return webapp?.version;
    }
    /// 查询一个资源包信息
    public func appLocalPath(withRoute url: String) -> String? {
        let webapp = selectWebapp(routeUrl: url)
        return webapp?.localPath;
    }
    /// 保存资源包信息
    ...
    LogWarning("url.absoluteString000\(url.absoluteString)")
        let callback = { (desUrl: URL?, error: Error?) in
LogWarning("url23233\(String(describing: url))")
            if let error = error {
                failure?(error)
        ...
        if let downloadingTask = downloadingTasks[url.absoluteString] {
            LogWarning("url.absoluteString111\(url.absoluteString)")
            downloadingTask.addCompletionCallback(callback)
            return downloadingTask
        }
        
        // 从路由表中查找路由URL
        LogWarning("urlxxxx\(String(describing: url))")
        if let routeUrl = Router.shared.routeUrl(with: url),
            let session = session {
             LogWarning("url.absoluteString222\(url)")
             LogWarning("url.routeUrl\(routeUrl)")
    ...
    fileprivate let sqlQueue: DispatchQueue = DispatchQueue(label: "TheOneHybrid.com.database")
...
extension ResourceManager: URLSessionDownloadDelegate {
    func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didFinishDownloadingTo location: URL) {
        ...
        if let url = downloadTask.originalRequest?.url?.absoluteString, let task = downloadingTasks[url] {
            LogWarning("url.absoluteString333\(url)")
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        if let url = task.originalRequest?.url?.absoluteString, let downloadingTask = downloadingTasks[url] {
             LogWarning("url.absoluteString444\(url)")
...
internal extension ResourceManager {
    ...
    fileprivate var sqlPath: URL? {
        if let webappPath = Util.webappPath {
            return webappPath.appendingPathComponent("TheOneHybrid.db")
        }
        return nil
    }
    ...
    fileprivate func initDatabase() {
        if let webappPath = Util.webappPath {
            let sqlUrl = webappPath.appendingPathComponent("TheOneHybrid.db")
